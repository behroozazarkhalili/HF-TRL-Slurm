#!/bin/bash
#SBATCH --job-name=train-carnice-9b-fable-glint
...
# Real training: Carnice-9B on FABLE-5 (Fable5-Glint, 4,350 rows, 3 epochs).
...
OUTPUT_DIR="/scratch/$USER/outputs/fable-train-carnice_9b-$(date +%Y%m%d)"
VENV="/scratch/ermia/venvs/hf_unsloth"
mkdir -p "$OUTPUT_DIR"

echo "=========================================="
echo "TRAIN: Carnice-9B on FABLE-5 (Fable5-Glint)"
...
NB_INPUT="$NB_DIR/fable_distillation_carnice-9b_fable-glint_unsloth.ipynb"
NB_OUTPUT="$OUTPUT_DIR/fable_carnice_9b_b_train_output.ipynb"
