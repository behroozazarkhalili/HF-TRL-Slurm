#!/bin/bash
# Qwen2.5-14B-Instruct GRPO Training on OpenR1-Math-220k

#SBATCH --job-name=qwen2.5-14b-grpo-openr1math
...
HUB_MODEL_ID="ermiaazarkhalili/Qwen2.5-14B-Instruct-GRPO-OpenR1Math"
GGUF_REPO_ID="ermiaazarkhalili/Qwen2.5-14B-Instruct-GRPO-OpenR1Math-GGUF"
...
python /project/6014832/ermia/HF-TRL/.claude/skills/slurm-model-trainer/scripts/train_grpo.py \
    ...
    --report_to trackio --trackio_dir $OUTPUT_DIR/trackio --project "grpo-openr1math" --run_name "qwen2.5-14b-grpo-$SLURM_JOB_ID"

[[ $? -ne 0 ]] && exit 1

python /project/6014832/ermia/HF-TRL/.claude/skills/slurm-model-trainer/scripts/generate_model_card.py \
    --model_name "Qwen2.5-14B-Instruct-GRPO-OpenR1Math" --base_model "$MODEL_NAME" --dataset "$DATASET_NAME" \
