#!/bin/bash
# Qwen3-8B GRPO Training on OpenR1-Math-220k

#SBATCH --job-name=qwen3-8b-grpo-openr1math
...
HUB_MODEL_ID="ermiaazarkhalili/Qwen3-8B-GRPO-OpenR1Math"
GGUF_REPO_ID="ermiaazarkhalili/Qwen3-8B-GRPO-OpenR1Math-GGUF"
...
export OUTPUT_DIR=$SCRATCH/outputs/qwen3-8b-grpo-$SLURM_JOB_ID
...
python /project/6014832/ermia/HF-TRL/.claude/skills/slurm-model-trainer/scripts/train_grpo.py \
    ...
    --report_to trackio --trackio_dir $OUTPUT_DIR/trackio --project "grpo-openr1math" --run_name "qwen3-8b-grpo-$SLURM_JOB_ID"

[[ $? -ne 0 ]] && exit 1

python /project/6014832/ermia/HF-TRL/.claude/skills/slurm-model-trainer/scripts/generate_model_card.py \
    --model_name "Qwen3-8B-GRPO-OpenR1Math" --base_model "$MODEL_NAME" --dataset "$DATASET_NAME" \
