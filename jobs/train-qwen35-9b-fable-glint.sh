#!/bin/bash
#SBATCH --job-name=train-qwen35-9b-fable-glint
...
# Real training: Qwen3.5-9B on FABLE-5 (Fable5-Glint, 4,350 rows, 3 epochs).
...
OUTPUT_DIR="/scratch/$USER/outputs/fable-train-qwen35_9b-$(date +%Y%m%d)"
VENV="/scratch/ermia/venvs/hf_unsloth"
mkdir -p "$OUTPUT_DIR"

echo "=========================================="
echo "TRAIN: Qwen3.5-9B on FABLE-5 (Fable5-Glint)"
...
NB_INPUT="$NB_DIR/fable_distillation_qwen35-9b_fable-glint_unsloth.ipynb"
NB_OUTPUT="$OUTPUT_DIR/fable_qwen35_9b_b_train_output.ipynb"
