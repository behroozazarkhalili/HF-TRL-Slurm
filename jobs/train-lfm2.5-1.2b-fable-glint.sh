#!/bin/bash
#SBATCH --job-name=train-lfm2.5-1.2b-fable-glint
...
# Real training: LFM2.5-1.2B on FABLE-5 (Fable5-Glint, 4,350 rows, 3 epochs).
...
OUTPUT_DIR="/scratch/$USER/outputs/fable-train-lfm25_12b-$(date +%Y%m%d)"
VENV="/scratch/ermia/venvs/hf_unsloth"
mkdir -p "$OUTPUT_DIR"

echo "=========================================="
echo "TRAIN: LFM2.5-1.2B on FABLE-5 (Fable5-Glint)"
...
NB_INPUT="$NB_DIR/fable_distillation_lfm2.5-1.2b_fable-glint_unsloth.ipynb"
NB_OUTPUT="$OUTPUT_DIR/fable_lfm25_12b_b_train_output.ipynb"
