#!/bin/bash
#SBATCH --job-name=train-fastcontext-4b-rl-fable-glint
...
# Real training: FastContext-4B-RL on FABLE-5 (Fable5-Glint, 4,350 rows, 3 epochs).
...
OUTPUT_DIR="/scratch/$USER/outputs/fable-train-fastcontext_4b_rl-$(date +%Y%m%d)"
VENV="/scratch/ermia/venvs/hf_unsloth"
mkdir -p "$OUTPUT_DIR"

echo "=========================================="
echo "TRAIN: FastContext-4B-RL on FABLE-5 (Fable5-Glint)"
...
NB_INPUT="$NB_DIR/fable_distillation_fastcontext-4b-rl_fable-glint_unsloth.ipynb"
NB_OUTPUT="$OUTPUT_DIR/fable_fastcontext_4b_rl_b_train_output.ipynb"
