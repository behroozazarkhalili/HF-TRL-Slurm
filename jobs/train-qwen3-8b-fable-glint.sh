#!/bin/bash
#SBATCH --job-name=train-qwen3-8b-fable-glint
...
# Real training: Qwen3-8B on FABLE-5 (Fable5-Glint, 4,350 rows, 3 epochs).
...
OUTPUT_DIR="/scratch/$USER/outputs/fable-train-qwen3_8b-$(date +%Y%m%d)"
VENV="/scratch/ermia/venvs/hf_unsloth"
mkdir -p "$OUTPUT_DIR"

echo "=========================================="
echo "TRAIN: Qwen3-8B on FABLE-5 (Fable5-Glint)"
...
NB_INPUT="$NB_DIR/fable_distillation_qwen3-8b_fable-glint_unsloth.ipynb"
NB_OUTPUT="$OUTPUT_DIR/fable_qwen3_8b_b_train_output.ipynb"
