#!/bin/bash
#SBATCH --job-name=train-gemma4-e4b-fable-glint
...
# Real training: Gemma4-E4B on FABLE-5 (Fable5-Glint, 4,350 rows, 3 epochs).
...
OUTPUT_DIR="/scratch/$USER/outputs/fable-train-gemma4_e4b-$(date +%Y%m%d)"
VENV="/scratch/ermia/venvs/hf_unsloth"
mkdir -p "$OUTPUT_DIR"

echo "=========================================="
echo "TRAIN: Gemma4-E4B on FABLE-5 (Fable5-Glint)"
...
NB_INPUT="$NB_DIR/fable_distillation_gemma4-e4b_fable-glint_unsloth.ipynb"
NB_OUTPUT="$OUTPUT_DIR/fable_gemma4_e4b_b_train_output.ipynb"
