#!/bin/bash
# DeepSeek-R1-Distill-Qwen-1.5B GRPO Training on OpenR1-Math-220k

#SBATCH --job-name=deepseek-r1-1.5b-grpo-openr1math
...
HUB_MODEL_ID="ermiaazarkhalili/DeepSeek-R1-Distill-Qwen-1.5B-GRPO-OpenR1Math"
GGUF_REPO_ID="ermiaazarkhalili/DeepSeek-R1-Distill-Qwen-1.5B-GRPO-OpenR1Math-GGUF"
...
export OUTPUT_DIR=$SCRATCH/outputs/deepseek-r1-1.5b-grpo-$SLURM_JOB_ID
...
python /project/6014832/ermia/HF-TRL/.claude/skills/slurm-model-trainer/scripts/train_grpo.py \
    ...
    --report_to trackio --trackio_dir $OUTPUT_DIR/trackio --project "grpo-openr1math" --run_name "deepseek-r1-1.5b-grpo-$SLURM_JOB_ID"

[[ $? -ne 0 ]] && exit 1

python /project/6014832/ermia/HF-TRL/.claude/skills/slurm-model-trainer/scripts/generate_model_card.py \
    --model_name "DeepSeek-R1-Distill-Qwen-1.5B-GRPO-OpenR1Math" --base_model "$MODEL_NAME" --dataset "$DATASET_NAME" \
