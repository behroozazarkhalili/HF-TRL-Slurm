#!/bin/bash
#SBATCH --job-name=smoke-qwen2.5-3b-grpo
...
export OUTPUT_DIR=$SCRATCH/outputs/smoke-qwen2.5-3b-grpo-$SLURM_JOB_ID
...
echo "=== SMOKE TEST: ermiaazarkhalili/Qwen2.5-3B-SFT-UltraChat ==="
echo "Node: $SLURMD_NODENAME | Start: $(date)"
echo "Note: SFT'd on UltraChat, now GRPO on NuminaMath"
...
PYTHONUNBUFFERED=1 python /project/6014832/ermia/HF-TRL/.claude/skills/slurm-model-trainer/scripts/train_grpo.py \
    --model_name_or_path ermiaazarkhalili/Qwen2.5-3B-SFT-UltraChat \
    ...
    --logging_steps 5 \
    --report_to none \
    --run_name "smoke-qwen2.5-3b-grpo" \
    --streaming \
    --max_samples 100 \
    --seed 42 \
