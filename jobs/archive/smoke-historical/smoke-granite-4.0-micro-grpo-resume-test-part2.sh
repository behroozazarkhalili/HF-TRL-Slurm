#!/bin/bash
#SBATCH --job-name=granite-resume-test-p2
...
echo "=== PART 2: Continual training from Part 1 adapter ==="
...
PYTHONUNBUFFERED=1 python /project/6014832/ermia/HF-TRL/.claude/skills/slurm-model-trainer/scripts/train_grpo.py \
    ...
    --logging_steps 5 \
    --report_to none \
    --run_name "granite-resume-test-p2" \
    --streaming \
    --max_samples 100 \
    --seed 43 \
    ...
    --max_steps 15 \
    --continue_from_adapter $OUTPUT_DIR
