#!/bin/bash
# =============================================================================
# granite-4.0-micro GRPO Continual Training on NuminaMath-CoT (20K new samples)
# Loads adapter from 10K run and trains on NEW data with different seed
# Total training: 10K (Part 1) + 20K (Part 2) = 30K samples
# =============================================================================

#SBATCH --job-name=granite-micro-continue-20k
...
MAX_SAMPLES=20000
SAMPLE_SIZE_LABEL="30K"  # Total: 10K (Part 1) + 20K (Part 2)

HUB_MODEL_ID='ermiaazarkhalili/granite-4.0-micro-GRPO-NuminaMath-30K'
GGUF_REPO_ID='ermiaazarkhalili/granite-4.0-micro-GRPO-NuminaMath-30K-GGUF'

# Adapter from the 10K run (job 33301494)
ADAPTER_DIR='/scratch/ermia/outputs/granite-4.0-micro-grpo-numinamath-cot-33301494'

# Training parameters (same as 10K run)
BATCH_SIZE=2
GRAD_ACCUM=8
LEARNING_RATE=1e-06
NUM_EPOCHS=1
...
echo ""
echo "=== CONTINUAL TRAINING ==="
echo "  Adapter from: $ADAPTER_DIR (10K samples, seed=42)"
echo "  New samples:  $MAX_SAMPLES (seed=43)"
echo "  Total:        30K samples"
echo ""
...
export OUTPUT_DIR="$SCRATCH/outputs/granite-4.0-micro-grpo-numinamath-cot-continue-$SLURM_JOB_ID"

# Log MIG status for debugging
echo "CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES"
if [[ "$CUDA_VISIBLE_DEVICES" == MIG* ]]; then
    echo "MIG device detected"
fi
...
if [[ ! -f "$ADAPTER_DIR/adapter_model.safetensors" ]]; then
    ...
    echo "Part 1 may not have completed. Contents:"
    ls -la "$ADAPTER_DIR/" 2>/dev/null
    exit 1
fi
echo "Adapter verified: $(ls -lh $ADAPTER_DIR/adapter_model.safetensors)"
...
# =============================================================================
# Phase 1: Continual GRPO Training
# =============================================================================
echo "=========================================="
echo "Phase 1: Continual GRPO Training (20K new samples)"
echo "=========================================="
...
PYTHONUNBUFFERED=1 python /project/6014832/ermia/HF-TRL/.claude/skills/slurm-model-trainer/scripts/train_grpo.py \
    ...
    --run_name "granite-4.0-micro-grpo-numinamath-continue-$SLURM_JOB_ID" \
    --streaming \
...
python /project/6014832/ermia/HF-TRL/.claude/skills/slurm-model-trainer/scripts/generate_model_card.py \
    --model_name "granite-4.0-micro-GRPO-NuminaMath-30K" \
...
echo "  Adapter Source:  $ADAPTER_DIR (10K run)"
...
echo "  Part 1: 10K samples (seed=42) → $ADAPTER_DIR"
echo "  Part 2: 20K samples (seed=43) → $OUTPUT_DIR"
echo "  Total:  30K samples"
echo ""
